...
export default function AdminDashboard() {
  ...
  async function fetchDashboardData() {
    ...
    if (profilesResponse.error || !profilesResponse.data) {
      ...
      setNotification('Błąd pobierania profili.');
      return;
    }

    const mappedProfiles = (profilesResponse.data as SupabaseProfile[]).map(mapSupabaseProfile);
    ...
    const premiumActiveCount = userProfiles.filter((profile) => isProfilePremium(profile)).length;
    ...
    setStats({
      ...
      active24h: userProfiles.filter((profile) => isWithinHours(profile.lastActive || profile.createdAt, 24))
        .length,
      newUsers7d: userProfiles.filter((profile) => isWithinDays(profile.createdAt, 7)).length,
      blockedUsers: userProfiles.filter((profile) => profile.isBlocked).length,
      verifiedUsers: userProfiles.filter((profile) => profile.isVerified).length,
      pendingSelfie: userProfiles.filter((profile) => profile.verificationPending).length,
